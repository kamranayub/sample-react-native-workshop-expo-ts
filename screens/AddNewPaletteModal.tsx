...
interface AddNewPaletteModalProps {
...
const AddNewPaletteModal = ({ navigation }: AddNewPaletteModalProps) => {
  ...
  const handleSubmit = React.useCallback(() => {
    if (!name) {
      ...
      const newColorPalette: ColorPalette = {
        id: -1,
        paletteName: name,
        colors: COLORS.filter(c => selectedColors.includes(c.colorName)),
      };
  ...
  const handleValueChange = React.useCallback(
    (value: boolean, { colorName }: Color) => {
      if (value === true) {
        setSelectedColors(prevColors => [...prevColors, colorName]);
      } else {
        setSelectedColors(prevColors =>
          prevColors.filter(selectedColor => selectedColor !== colorName),
        );
      }
    },
    [setSelectedColors],
      ...
      <FlatList
        ...
        renderItem={({ item: color }) => (
          <View style={styles.color}>
            ...
            <Switch
              value={selectedColors.includes(color.colorName)}
              onValueChange={value => handleValueChange(value, color)}
            />
          </View>
        )}
      />
